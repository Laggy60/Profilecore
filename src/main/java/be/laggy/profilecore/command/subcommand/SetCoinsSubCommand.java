...
public class SetCoinsSubCommand {

    private final Profilecore plugin;

    public SetCoinsSubCommand(Profilecore plugin) {
        this.plugin = plugin;
    }

    public boolean execute(CommandSender sender, String[] args) {
        if (!PermissionUtil.has(sender, Settings.PERMISSION_SET_COINS, Settings.PERMISSION_ADMIN)) {
            ChatUtil.send(sender, plugin.getMessageManager().get("no-permission"));
            return true;
        }

        if (args.length < 3) {
            ChatUtil.send(sender, plugin.getMessageManager().get("usage-setcoins"));
            return true;
        }
        ...
        ChatUtil.send(sender, plugin.getMessageManager().get("set-coins-success")
