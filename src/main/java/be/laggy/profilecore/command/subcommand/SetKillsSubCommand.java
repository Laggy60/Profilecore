...
public class SetKillsSubCommand {

    private final Profilecore plugin;

    public SetKillsSubCommand(Profilecore plugin) {
        this.plugin = plugin;
    }

    public boolean execute(CommandSender sender, String[] args) {
        if (!PermissionUtil.has(sender, Settings.PERMISSION_SET_KILLS, Settings.PERMISSION_ADMIN)) {
            ChatUtil.send(sender, plugin.getMessageManager().get("no-permission"));
            return true;
        }

        if (args.length < 3) {
            ChatUtil.send(sender, plugin.getMessageManager().get("usage-setkills"));
            return true;
        }
        ...
        profile.setKills(amount);
        plugin.getProfileService().saveProfileAsync(profile);

        ChatUtil.send(sender, plugin.getMessageManager().get("set-kills-success")
